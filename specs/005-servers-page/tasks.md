...
- [X] T006 Add the four tables to `SCHEMA` in `app/db.py` per [data-model.md](./data-model.md): `steam_trade_links`, `payments` (with `UNIQUE (method, provider_ref)`), `credit_ledger`, `servers`. All `CREATE TABLE IF NOT EXISTS`, additive only, so `init_schema` stays idempotent and no `ALTER TABLE` path is needed for deployed databases
